...
const SmsRecord = () => {
  ...
  const columns = [
    ...
    {
      title: "SMS & Caster Number",
      dataIndex: "smsCasterNumber",
      key: "smsCasterNumber",
      filterable: true, // Enable search and filter
  ...
  const populateData = async () => {
    ...
    try {
      const { data } = await apiCall("POST", "/sms/getSmsSummary", token, filter);
      const processedData = data?.responseData?.map((item) => ({
        ...item,
        smsCasterNumber: `${item.smsNumber || ""} , ${item.casterNumber || ""}`,
      }));
        ...
        <div>
          ...
          >
            ...
            <CustomDatePicker
              ...
              required
            />
            <Btn htmlType="submit" className="w-full"> Search </Btn>
            <Button className="flex gap-2 items-center border-darkBlue text-darkBlue" onClick={() => window.location.reload()}>
              <span>
                <CloseCircleOutlined />
              </span>
